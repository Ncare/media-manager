...
log = logging.getLogger("media_manager.renamer")
...
# Generic NFO names that don't tied to a specific media filename — kept as-is.
_GENERIC_NFO_NAMES = {"movie.nfo", "tvshow.nfo"}
...
def _move_companions(
    ...
    *,
    ...
    If src_stem/dest_stem are given, a per-file NFO named ``{src_stem}.nfo`` is
    renamed to ``{dest_stem}.nfo`` so it follows the media file's new name.
    """
    if not src_dir.exists() or not src_dir.is_dir():
        return []
    moved: list[tuple[str, str]] = []
    dest_dir.mkdir(parents=True, exist_ok=True)
    same_dir = src_dir.resolve() == dest_dir.resolve()
    for entry in src_dir.iterdir():
        if not entry.is_file() or not _is_companion(entry):
            continue
        # Determine the target name. Per-media NFOs (e.g. "Inception.nfo")
        # follow the media file's new stem; generic NFOs keep their name.
        target_name = entry.name
        if (
            entry.suffix.lower() == ".nfo"
            and src_stem
            and dest_stem
            and entry.stem.lower() == src_stem.lower()
            and entry.name.lower() not in _GENERIC_NFO_NAMES
        ):
            target_name = f"{dest_stem}.nfo"

        target = dest_dir / target_name
        if target.exists() and target.resolve() != entry.resolve():
            continue  # don't overwrite an existing companion at destination
        if same_dir and target.resolve() == entry.resolve():
            continue  # same file, nothing to do (rename in place with same name)
        try:
            os.rename(entry, target)
            moved.append((str(entry), str(target)))
        except OSError as e:
            log.debug("companion move failed %s -> %s: %s", entry, target, e)
    if moved:
        log.info("moved %d companion(s): %s -> %s", len(moved), src_dir, dest_dir)
    return moved
...
def execute(
    ...
    for mid in media_ids:
        ...
        # fanart.jpg, *.nfo next to the file). Pass the old/new stems so a
        # per-file NFO ({filename}.nfo) follows the media's new name.
        companions = _move_companions(
            src.parent, dest.parent,
            src_stem=src.stem, dest_stem=dest.stem,
        )
